...
- **Editor Nova copilot drawer exception:** the full-screen editor's Nova tool may use the creator-agent conversation grammar because it is a command/receipt surface, not an onboarding interview. User commands use the shared ink bubble; assistant replies use shared unboxed prose. Change chips remain `border-lime-200 bg-lime-50 text-lime-800`; rejected chips `border-dashed border-zinc-300 bg-white text-[#71717a]`; suggestion chips `border-zinc-200 bg-white` with lime hover/focus.
