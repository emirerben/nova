...
- **Landing story rhythm:** `/` uses one pinned `760svh` stage; `/auto-story` uses the same composition in one viewport. Source footage and feature chips surround the centered screen, then travel directly into it before the three statements replace one another.
