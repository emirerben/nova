...
- **CTA (InkButton):** ink pill `rounded-full bg-[#0c0c0e] px-9 py-[15px] text-[15px] font-semibold text-white hover:opacity-80`.
  **Single-primary-CTA rule on landing:** one CTA to `/plan` after the edit story — never a second CTA alongside it.
...
- **Landing story rhythm:** `/` defaults to the timed, automatic composition in one viewport; `/?mode=scroll` uses the same composition in one pinned `760svh` stage. `/auto-story` remains a compatibility route for the automatic version. Source footage and feature chips surround the centered screen, then travel directly into it before the three statements replace one another.
...
- **Header:** product routes get sticky scroll-fade header (`rgba(0,0,0,0.6·progress)` + blur); landing routes (`/`, `/auto-story`) get the static cream header; `/admin` hides Header entirely.
...
| Landing edit story | `--story-move-dur: 500ms`, `--story-overlay-stagger: 60ms`, `--story-reduced-dur: 200ms`, `--story-ease-move`, `--story-ease-out` | `/` timed choreography by default, with the scroll comparison at `/?mode=scroll` and `/auto-story` retained as a compatibility route. Movement is transform/opacity only; reduced motion removes positional travel. Navigation, CTA, and ambient background remain static. |
