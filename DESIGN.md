...
- **Chat-first motion and a11y:** format cards are keyboard-selectable, the composer submits with Enter (Shift+Enter inserts a newline), focus remains visible at 200% zoom, and reduced motion removes entrance travel while preserving state changes. Use `video/*,image/*,audio/*` for the unified picker; narrated formats reuse the existing voice recorder.
