...
test('discovery uses the npm executable environment', async () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(readFileSync(calls, 'utf8')), ['exec', '--no', '--', 'jest', '--listTests', '--json', '--runInBand']);
  } finally {
