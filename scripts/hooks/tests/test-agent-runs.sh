...
assert set(record) == {"founder", "tool", "ticket", "started_at", "finished_at", "elapsed_seconds", "outcome"}
...
assert [record["tool"] for record in records] == ["codex", "claude"]
PY
...
# The recap totals locally reported finishes once per local date. The marker
