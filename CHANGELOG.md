...
### Changed
- **Layer-2 cache invalidation is now content-hashed — manual `TEXT_OVERLAY_VERSION_V2` bumps are no longer required.** The old pattern was a hand-edited string constant (`v2-2026-05-22-atomized-single-word`) that every Stage-E/D/G change had to remember to bump; a missed bump silently served stale recipes. Five PRs in seven days were spent on Layer-2 cache invalidation (#221, #249, #250, #252, #261). The constant is replaced by `compute_text_overlay_version()`, which derives a `v2-<hex16>` string from the SHA-256 of every Layer-2 prompt file, the SHA-256 of every Layer-2 schema module, the Layer-2 agents' `AgentSpec.prompt_version` strings, and a sorted JSON dict of the relevant settings flags. Any of those changing produces a fresh cache namespace automatically — including the Stage-E/G changes that landed in v0.4.42.4 (#288), which the hash now reflects without a manual bump. Existing `from app.pipeline.template_cache import TEXT_OVERLAY_VERSION_V2` callers keep working via a module-level `__getattr__` that resolves the symbol to the live computed value.
