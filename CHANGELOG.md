...
### Fixed
- **Progressive word-by-word reveal now actually shows the full source phrases instead of fragmenting them.** Three related Layer-2 bugs were causing the rendered video to show isolated single-word flashes ("It's" → "not" → "luck\"") instead of cumulative reveals of the on-screen phrases ("It's" → "It's not" → "It's not just" → "It's not just luck"). Designed against the failure pattern observed in prod template `89cde014-f096-4625-881f-820e09524d71` and its two most-recent jobs (final pixel-level validation requires post-deploy reanalyze + render):
  ...
  - **(C) `build_line_groups` skips unmatched phrases mid-group instead of closing.** This was the killer: even with (B) filtering most artifacts out, any single phrase the transcript happened to miss (e.g. `"there"` when Whisper skipped it) would close the running group at `line_grouping.py:194-198`. Pre-fix, job `09f56ee3` (07:42, post-good-transcript build) emitted the partial cumulative reveal `"The"` / `"The work"` / `"The work to"` / `"The work to get"` and then dropped `"there"` as a standalone single-word overlay because an unmatched phrase between them closed the group. Worse, the second half of `"don't allow anyone to diminish your hard work"` only managed `"allow"` / `"allow anyone"` before fragmenting. Fix: unmatched phrases are now SKIPPED — they neither enter the group's `phrase_indices` nor close it. Groups close only on real terminators (sentence-terminating punctuation in the transcript span between matched words, silence gap above `silence_gap_s`, or `max_words_per_line` cap). Boundary detection still operates against the last matched phrase, so a real silence gap or sentence end across an artifact-filled span still closes correctly.
