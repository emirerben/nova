...
### Fixed
- **Admin "Reanalyze" on audio-only templates now returns a clear error instead of a 9-frame-deep GCS unicode crash.** Music tracks promoted to templates (`template_type="audio_only"`, created at `admin.py:2450` from a `MusicTrack`) have `gcs_path=None` by design — their recipe is pre-generated from beat detection on the audio. Clicking the manual "Reanalyze" button on one of these rows enqueued `analyze_template_task`, which read `template.gcs_path → None` and handed it directly to `storage.download_to_file(None, ...)`. The job died eight frames deep inside `google-cloud-storage`'s `Blob.__init__` with `ValueError("None could not be converted to unicode")` — a message that took a full /investigate cycle to attribute to the actual mistake (caller forgot to null-guard a nullable column). Three layers of guard, smallest-blast-radius outward:
  - `routes/admin.py` reanalyze route now mirrors the existing `is_agentic` 409: an audio-only template gets a clear "Use POST /admin/music-tracks/{track_id}/reanalyze to re-detect beats and regenerate the recipe" message instead of being enqueued at all.
  - `tasks/template_orchestrate.py::analyze_template_task` adds a fail-fast guard at task entry — if the row somehow reaches the worker with `template_type="audio_only"` or `gcs_path=None`, the task sets `analysis_status="failed"` with the same operator-facing message and returns immediately, never touching GCS.
  - `storage.py::download_to_file` rejects `object_path=None` at the boundary with a message that names the actual mistake. Defense in depth — catches the bug class wherever else it might surface in the codebase, not just this one caller. Two regression tests pin both the route's 409 + the storage helper's `ValueError`.
