...
### Changed
- feat(ios): offer every web font in the editor and draw each choice in its own typeface (KRI-171) (#1193) <!-- release-pr: 1193 -->
...
### Changed
- feat(ios): add a kind-aware narration/voiceover analysis-proxy contract (KRI-93) (#1050) <!-- release-pr: 1050 -->
...
### Changed
- fix(ios): trust OpenAPI archive plugin (#1035) <!-- release-pr: 1035 -->
...
### Changed
- fix(ios): make editor pause and resume respond without redundant rendering (#1029) <!-- release-pr: 1029 -->
...
### Changed
- fix: prevent false draft conflicts and allow idle project deletion (KRI-35) (#1014) <!-- release-pr: 1014 -->
...
### Fixed
- **Kria can now continue editing large guided-story videos in the first chat turn.** Once the API advertises compact-snapshot support, oversized editor snapshots stay within the request budget without losing clip identity, lyric locks, guided titles, or motion controls; older API deployments keep receiving the legacy complete snapshot during rollout.
...
### Changed
- **Supersedes #870's UI direction** (founder call): the flagged `/create` hub and
...
- **Stories now fit your footage instead of demanding 10+ seconds of it.** The guided planner's hard 10-second minimum is gone; the target length clamps to what your clips can actually support (renderer-aware feasibility, no slow-mo stretching), measured before the AI ever drafts.
...
### Changed
- **The landing page now keeps one clear next step in its original story position.** The header is borderless and no longer shows Sign in, while “Create my first edit” remains centered near the bottom of the story with Terms and Privacy in a quiet line beneath it. The previous process section and footer remain removed.
...
- **TikTok publication status and authorized performance now appear in the Library.** Nova tracks upload processing separately from moderation and visibility, reconciles signed webhooks with fallback polling, and displays views, likes, comments, and shares returned for linked posts. Performance learning still waits for confirmed public visibility and maturity. Durable recovery prevents queue interruptions or ambiguous TikTok timeouts from creating silent duplicate posts.
...
- **Behind-subject text no longer strobes under heavy moving occlusion.** Restores the anti-strobe visibility policy (shipped in 0.7.31.2, removed in 0.9.3.0 — the CHANGELOG entries for those two releases describe mutually exclusive behaviors; both now hold): the layer hides with hysteresis and a 3-frame fade when occlusion is near-total (>98%) or heavy (>70%) **and** measurably strobing (repeated frame-to-frame visible-alpha jumps). A smooth single-subject sweep — the reason 0.9.3.0 removed the policy — stays strictly per-pixel, pinned by the unchanged partial-sweep test.
...
### Added
- **Smart Captions now builds the complete Çiğdem-style composition from transcript meaning and the creator's visual pool.** Hook assets accumulate on screen, numbered chapter titles render independently from subtitles, contextual visuals can take over the frame, semantic crop pulses share the existing reframe encode, and typewriter reveals produce synchronized keyboard ticks.
- **Caption correction is grounded and timing-safe.** Smart Captions can repair phonetic brand or character-name errors only from sanitized, visual-pool-backed aliases; untrusted rewrites are rejected while word IDs and timestamps remain unchanged. Existing non-Smart caption correction keeps its original behavior.
...
### Added
- **"Subtitled" edit style — single-clip auto-captions (Turkish + English).** Third user-facing edit style after Montage and Narrated walkthrough: upload one talk-to-camera clip, Nova transcribes its OWN audio (whisper-1, spoken language auto-detected) and burns editable captions over a cached caption-free base. A gpt-4o correction pass fixes ASR mishearings per cue — spelling, grammar, and Turkish case/suffix errors ("Kaçer"→"Kaçar", "nereye"→"nereyi") — under an N-line guard so cue timing never shifts; cues then re-split one sentence per caption (word-timing-aligned, tail fragments merged, Turkish ordinals kept whole) with a fade+scale pop-in, burned at a platform-safe MarginV (384/1920) clear of the TikTok/Reels UI. Two caption styles: sentence blocks (default) and word-by-word with the active word popped in the Nova lime (line-visible libass events from real word timings; synthesized only for edited cues). Editing reuses the on-video CaptionEditor (review-first banner, language chip with confirm re-transcribe via a new `caption-language` endpoint + Celery task, font picker, 44px targets); the preview mirrors the real burn geometry. New pipeline modules: `caption_correct.py` (LLM pass, traced for admin job-debug) and `captions.resplit_cues_into_sentences` / `generate_word_pop_ass`. Flags: `SUBTITLED_ARCHETYPE_ENABLED` (Fly) + `NEXT_PUBLIC_SUBTITLED_ENABLED` (Vercel) — both default **off**; companions `SUBTITLED_CAPTION_CORRECTION_ENABLED` / `CAPTION_CORRECTION_MODEL` (default gpt-4o; mini failed Turkish contextual grammar 4/4 runs). Hardened in review before merge (5 specialists + adversarial + red team, ~40 findings actioned): caption payloads bounded at the edge (text ≤600 chars, ≤100 words/cue — word-pop is O(tokens²)); caption dispatchers close the 409 gate synchronously AND commit it (get_db rolls back otherwise); a zero-cue re-transcribe keeps existing captions instead of destroying edits; the generic re-render hard-rejects caption variants (montage-clobber guard); word-pop events are monotonic (stacked-text incident class); superseded burns are deleted from GCS (generative-jobs/* never expires); subtitled clips capped at 5 min so caption edits fit the 600s task budget; transcription auto-detect + per-word timings survive the sentence re-split end-to-end.
...
### Added
- **Editorial word-cluster intro for generative edits.** The hook can now render as a magazine-style word cluster — separate blocks with mixed sizes at offset positions, fading in one after another (the aesthetic-travel-edit look: small "what's your", huge Didone "favorite", offset "place?") — instead of always one centered linear block. `overlay_format_matcher` gained `layout: linear|cluster` (cluster examples added to `overlay_examples.json`; never paired with karaoke — parse coerces to `fade-in`); `intro_writer` gained `word_roles` (hero/connector/closer per word, validated against the sanitized text, closer strictly final). Geometry is NOT agent-decided: a new deterministic layout engine (`app/pipeline/intro_cluster.py`) computes per-block `position_x_frac`/`position_y_frac`/`text_size_px`/`font_family` from real Skia glyph measurement — cluster-atomic shrink preserves the size hierarchy, every block is clamped inside frame margins by construction, and unsuitable text (word count ∉ [3,6], unfittable hero words) declines to the proven linear intro. Each block renders as an ordinary [bounded fade-in reveal, static hold-to-EOF] overlay pair using ONLY existing renderer fields — zero renderer changes, renderer-parity-safe, frame budget ≤ a single karaoke intro. Connectors render in the regular weight of the hero face ("Playfair Display Regular"). Threaded through all four burn paths via `_resolve_intro_overlay_params`; effective `intro_layout` + `intro_word_roles` persist on the variant so retext/swap-song/restyle re-renders rebuild the cluster with no LLM call (user-typed override text drops stale roles; the engine re-derives heuristically). Any explicit position (user knob, curated set, or non-center advisory) forces linear. The instant text editor excludes cluster variants (`intro_layout` surfaced on the status response; the TS mirror only models the linear layout) — cluster text edits use the server fast-reburn path. Kill switch: `GENERATIVE_CLUSTER_INTRO_ENABLED` (default true). Verified: `make verify-overlays` PASS (cluster fixture, un-clipped per block), full generative local-render E2E, and real-footage cluster burns through the prod image in EN + TR. Follow-up (TODOS.md): the reference's narrative bottom-caption arc across body clips.
- **Post-render layout pick (Classic / Editorial).** Every AI-text variant card gains a layout toggle: the user can flip a rendered variant between the classic linear intro and the editorial word-cluster after generation — one fast reburn (~seconds, no LLM; persisted text reused, roles re-derived when switching into cluster). `EditVariantRequest.intro_layout` on the combined `/edit` endpoint → `regenerate_generative_variant(layout_override=…)`. Cluster on a >6-word hook is rejected with an actionable 422 ("shorten the text first") and the chip pre-disables client-side with the same hint. Wired on the public generative page and the admin generative detail page.

## [0.4.97.1] — 2026-06-11

### Fixed
- **Clip timeline editor never appeared (prod bug):** `_finalize_job` rebuilds every variant entry through an explicit whitelist when a generative job completes — `ai_timeline` wasn't in it, so the timeline the render persisted was stripped seconds later and every variant reported `no_timeline` (diagnosed against prod jobs 568ced7b/22c0bc36; the same strip class previously hit the fast-reburn fields, per the whitelist's own comment). Pinned by `test_finalize_job_preserves_ai_timeline`. Voiceover `mix` added to the whitelist too (same class — the slider state was being reset at finalization). Existing jobs heal on their next full re-render (e.g. swap-song); new renders carry timelines immediately.
- **Plan-items surface had no editor entry:** the backend mirror endpoints shipped in v0.4.97.0 but `/plan/items/[id]` renders variants through `PlanVariantEditor`, which never got the UI. Timeline API client + `useTimelineSession` + `TimelineEditor` are now base-aware (`generative` | `plan-item`); the plan page gets "Edit clips · N", the "Edited cut" pill, and the swap-song confirm when clip edits exist.
...
### Added
- **Creator Agent M1-FE: StyleCard in workspace.** Presentational card in the plan workspace left rail showing the user's derived style (set pill, font, color swatches, instruction level, footage bias). Five render states: absent → null, deriving → skeleton, ready/edited → pills, failed → muted. Links to `/plan/style` for edits. Ships together with the per-user Style entity from M1 (dark behind `USER_STYLE_ENABLED`).
- **Creator Agent M2: StyleIntentAgent + editorial-interview frontend (dark behind `STYLE_AGENT_ENABLED=false`).** `nova.plan.style_intent` parses a free-text style utterance into one of five typed intents (set_style, adjust_knob, set_footage_bias, set_instruction_level, general_vibe). New `/plan/style` page with an editorial interview UI (`StyleAgentInterview`). API routes: `POST /personas/agent/start`, `POST /personas/agent/turn` (404 when flag off).
- **Creator Agent M3: style-driven plan + filming guide in render.** Planner reads `instruction_level` + `preferred_edit_format_mix` from `UserStyle`, injecting contextual guidance per plan item. `intro_writer` receives a `filming_guide` (2–4 shot keys keyed to `edit_format`) → richer hook context. `_resolve_archetype` applies a soft `footage_type_bias` tiebreaker in the generative engine (transparent when no bias set). `CONTENT_PLAN_PROMPT_VERSION` → `2026-06-07`, `intro_writer` prompt version → `2026-06-07`.
- **Creator Agent M4: ConformanceFeedbackAgent at clip-attach time (dark behind `CONFORMANCE_FEEDBACK_ENABLED=false`).** Migration 0051: nullable `conformance` JSONB on `plan_items`. `nova.plan.conformance_feedback` (Gemini Flash) produces a `verdict` (on_track / minor_drift / off_brief), confidence, summary, mismatches, and suggestions. Fire-and-forget Celery task after `attach_clips` commit (best-effort, max_retries=0). Plan-item page shows a verdict panel with polling (max 3 extra cycles); never blocks Generate. Instructed items (instruction_level ≠ "none") get a single-file replace UI.
- **Generative-edits: stable intro text across font/size/style changes.** Intro text generated by `intro_writer` is now persisted on the variant (`intro_text`, `intro_highlight_word` fields). Subsequent font, size, or style edits reuse the persisted text — `intro_writer` only re-runs when there is genuinely no text yet (first render or legacy variant). Fixes the longstanding bug where changing the font silently rewrote the user's AI-generated text.
- **Generative-edits: fast-reburn path for style/font/size edits.** For `agent_text` variants, the assembly produces a text-free, audio-mixed base video that is cached to GCS (`generative-jobs/{job_id}/base_{rank}_{variant_id}.mp4`). Font, size, and style changes download the base and reburn only the text overlay via a single `preset=fast` FFmpeg pass — seconds instead of minutes, with no Gemini re-analysis and no clip re-download. Audio (incl. voiceover) is preserved from the cached base.
- **Kill switch: `GENERATIVE_FAST_REBURN_ENABLED`.** Defaults `true`. Set to `false` + restart workers to fall back to full re-render on every edit. Layer-1 text persistence stays active regardless.
- **Frontend: edit-text prompt prefills with current text.** The "Edit intro text" prompt now pre-populates with `variant.intro_text` so users see what text they're editing.
- **Endpoint mapping:** `retext` / `remove-text` / `change-style` / `intro-size` → fast path (seconds). `swap-song` / `mix` → full path (audio change invalidates the base; fresh base emitted for the next edit cycle).

### Changed
- `VariantCard`: edit prompt seeds from `variant.intro_text` (no payload changes — field already in the variants dict).
...
### Added
- **Stage-aware loading system across generative edits.** The generative flagship page now shows a full Phase Chips Theater (D3/D5) — a compact status band with phase chips (Queued → Analyzing → Matching song → Rendering → Done), a crossfading Playfair Display headline, a damped progress bar (D11: `pos = from + (to-from)·(1−e^(−1.6·elapsed/baseline))`), and honest ETA from server-side baselines. The payoff zone shows N shimmer tiles before the variant set is known (D7 ghost slots), lighting up with arrive animations as each variant finishes. No fabricated progress: every number shown is backed by a server timestamp or event (D6). Phase chips, bar, and tiles all snap to truth instantly on tab-return (D8). ETA uses a coarse ladder (≥90s / 25–90s / <25s / overrun) so rounding absorbs ±30% baseline variance (D18). Stall copy escalates at 1.5× and 2.5× per-phase baseline and reverts on the next real event (D19). Completion recedes to a receipt line "✓ Ready in m:ss" after a 1.2s celebration (D12). Failed tiles show a plain-language reason ("This render took too long and was stopped") with no raw FFmpeg text, and a Try again button (D10). Full reduced-motion + screen-reader contract: chips/bar/animations are static swaps under `prefers-reduced-motion`; `aria-valuenow` updates only on real events (D17).
- **Generative pipeline phase instrumentation.** `orchestrate_generative_job` now calls `mark_started`/`record_phase`/`mark_finished`/`mark_failed_phase` from `job_phases.py` (no new migration — columns already exist). Trunk phases: `queued → analyze_clips → match_song → render_variants → finalize`. Planned variants are upserted as `render_status: "pending"` at spec-decision time so the frontend knows the tile count from the moment variants are known (not incrementally). Per-variant `render_started_at`/`render_finished_at` timestamps power the tile elapsed clocks. Machine-readable `error_class` stamped at every failure site (`timeout`, `encoder_error`, `clip_read_error`, `storage_error`, `unknown`); raw exception string stays admin-only.
- **Hand-seeded phase baselines.** `app/services/phase_baselines.py` serves advisory per-phase durations (`analyze_clips=45s, match_song=15s, render_variants=30s×N, finalize=10s`) for ETA and anchor computation. Baselines scale by pending variant count. Refresh from real prod data is a tracked TODO once the instrumentation soaks.
- **Shared `ProgressTheater` primitive library.** `src/components/progress/` (ProgressTheater, PhaseChipRow, StatusHeadline, EtaBar, VariantRenderCard, PayoffField) + `components/progress/logic.ts` (all theater math as pure functions, DOM-free, Jest-targeted) + `hooks/usePolledJobStatus.ts` (shared poll hook with D8 visibilitychange refetch). Kills the third hand-rolled `setTimeout` poll loop on the generative page. `lib/job-phases.ts` generalizes the template phase vocabulary to support any pipeline; `lib/template-job-phases.ts` remains the re-export source of truth so the template screen compiles untouched.
...
### Added
- **Content plan items now include a structured filming guide.** Each item generated by `content_plan_generator` gets a 2–4 shot list (`{what, how, duration_s}`) keyed to its `edit_format` archetype — montage items get 3–4 varied cuts, talking_head items lead with the to-camera take, day_vlog items run in time order, and single_hero items focus on the hero subject. The guide is stored as JSONB on `plan_items` (migration 0047), returned read-only by the API, and rendered as a numbered shot card on the item detail page (`/plan/items/[id]`). Legacy items without a guide fall back to the existing one-liner `filming_suggestion`.
