...
- **Web CI no longer times out inside Radix's JSDOM pointer path.** Menu and select coverage now drives the same accessible keyboard contracts used by the components, and each interaction-heavy suite gets a fresh process before the remaining Jest suite without weakening deadlines.
