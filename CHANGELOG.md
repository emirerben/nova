...
### Added
- **Fast-preview toggle on the admin template test tab.** Cuts a 5-clip cold-cache test run from ~3 min to ~30-60s by gating three expensive stages behind a `preview_mode` flag set only by the admin endpoint. When on, the orchestrator (a) drops curtain-close interstitials before assemble, (b) skips `generate_copy` entirely (returns an empty `PlatformCopy` with `copy_status="skipped_preview_mode"`), and (c) **forces the single-pass encode path** (PR #147) regardless of the per-template `single_pass_enabled` allow-list — assemble was the long pole on agentic test templates that hadn't yet been parity-promoted to single-pass in prod, and `force_single_pass=True` via `apply_async` kwargs is the documented engineer-debug override for exactly this case. Threaded through both `_run_template_job` (multi-video) and `_run_single_video_job` (single-video) so it works on every template kind. Final-output encoder presets are untouched — pixel quality of the rendered video matches production. Toggle off in the UI to run the full pipeline. Default ON for the admin test tab; default OFF on the backend API so external/non-admin callers see zero behavior change.
