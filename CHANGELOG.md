...
- **Lines pinned near an edge no longer drift.** A few looks pin the text a fixed distance from the left edge, or park it near the bottom, and leave the other direction up to Nova. The editor was guessing the missing half wrong and drawing those lines about five percent of the frame too low. Worse, saving from that view made the wrong position permanent. Both the opening line and editable lyric lines now line up with the video.
- **Text pinned right against an edge stays there.** A line placed hard against the left edge, or a line Nova chose not to tilt, was treated as having no position at all and snapped back to centre. Edge positions are now kept exactly as set.
