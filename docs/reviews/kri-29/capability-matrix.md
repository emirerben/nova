...
| `stillImages` | `asset_manifest.assets[].kind == visual` (no `media_kind`, or `image`) | Creator Visuals-pool photos drawn as main-track stills (KRI-121). `compile_phone_guided_plan` only emits them for `kind == image`, `lane == asset` moments with no `image_motion`, look, crop or speed change, as `fullscreen` or as `supporting_card` (`clips[].still_layout`, which needs no capability of its own), and only while this bit is in `phone_render_verified_features`. The device downloads each photo through the per-asset grant for its pinned generation, verifies its SHA-256 and mattes transparency over black, as the cloud normalizer does. Image motion and looks on photos still reject. |
| `visualVideos` | `asset_manifest.assets[].kind == visual` with `media_kind == video` | Creator Visuals-pool videos drawn as fullscreen main-track clips (KRI-121). The worker hashes and probes each one at its pinned generation; the grant also checks the row's kind; the device downloads it like a photo, verifies its SHA-256 and hard-links it under a playable extension. Same treatment limits as bound footage (fullscreen, 1x, no crop; `golden_hour` only on exact-canvas unrotated sources). Gated separately from `stillImages` so photos and videos can be verified on device independently. |
...
| `narrationAudio` | `GuidedStoryExecutionPlan.narration`; `GenerativeVariantDecision.extras["voiceover_gcs_path"]` (montage/day_vlog/single_hero "voiceover" archetype) | Guided-story `.narration` stays rejected before a recipe can exist (`phone_guided_plan.py`'s `_UNSUPPORTED_PHONE_LANE_CAPABILITY`) — no schema/Python work has landed for it. KRI-132's `app.pipeline.phone_montage_plan.compile_phone_montage_plan` is a SEPARATE, second producer: a montage-family item with a recorded voiceover (never the guided-story lane) compiles a `VoiceoverRenderAsset` + a `TimelineTrack(id="narration", kind="audio")`, mirroring the cloud's `_mix_user_voiceover` gain math (`AudioMixRecipe.narration_asset_id`), behind `settings.phone_narration_rendering_enabled` (default `true` since the KRI-132 rollout) — the dispatch gate (`content_plan_build._dispatch_item_render`) fails closed with a typed `phone_voiceover_unavailable` reason before a Job is minted when the flag is off or this bit isn't verified. Still "local-later" here, same reasoning as `musicBed` below: no `KriaMediaEngine` native primitive has been verified to play a second audio-kind track from a private (non-catalog) asset yet — local-v1 is pending device-side verification, not schema/Python work. The `narrated`/`narrated_*` archetypes (side-chain-ducked original-audio bed) and guided-story narration remain out of scope and stay cloud-only regardless of this flag. |
