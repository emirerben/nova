...
| Audio | Original, licensed music/bed, narration, SFX; gain, trimming, ducking, mixes | Basic gains/mix supported; planner/catalog/source-role integration outstanding. KRI-132 adds a montage-family recorded-voiceover lane (`compile_phone_montage_plan`, `VoiceoverRenderAsset`, `AudioMixRecipe.narration_asset_id`) behind `PHONE_NARRATION_RENDERING_ENABLED` (default off) — footage/matched-track bed gain mirrors the cloud's `_mix_user_voiceover` exactly; the cloud's `loudnorm` normalization pass has no phone equivalent (a known, accepted difference). Side-chain-ducked original-audio narration beds (the `narrated`/`narrated_*` archetypes) and the guided-story narration lane remain cloud-only. |
