...
| `giant-title-wipe` | Native vector camera/opacity for static, none, fade-in, scale-up, slide-up/down/in, pop-in, bounce, ink-reveal, lyric-line, typewriter, stream-in, karaoke-line, smooth-type, staggered-slice and handwriting; dissolve compiles the static-source behavior used by production. 396 cloud frames plus preview/export/backward-seek tests pass | Full styles/languages and physical performance; [physical pilot](giant-title.md) has successful exports with intermittent AVFoundation interruptions |
...
| Looks | `faded_analog`, `golden_hour`, `none`, `olive_film`, `smoky_split_tone`, `stadium_diffusion` (`pipeline/look_presets.py`) | Golden hour compiled for exact-canvas unrotated footage; native SDR H.264 preview/export passes 24 small/full-canvas moving samples against measured conversion and encoding baselines. YUV/NV12 primitive matches all 393,216 reference bytes. General rollout unqualified; the six-second physical pilot passed. Resizing, other formats/looks and full performance gates remain open ([details](color-looks.md)) |
| Transitions | `crossfade`, `fade_black`, `fade_white`, `wipe_left`, `wipe_right` (`pipeline/transitions.py`), plus semantic hard/match cuts, speed ramps and curtain-close | Five native render paths; 40 production-FFmpeg grayscale references pass preview/export/backward seeking. 60 moving-SDR preview/export samples also pass. Guided compiler handles source-safe millisecond crossfade/dip/flash boundaries; general rollout remains unqualified. The six-second physical pilot passed; source audio, further color profiles and full performance gates remain open ([details](clip-transitions.md)) |
...
| Audio | Original, licensed music/bed, narration, SFX; gain, trimming, ducking, mixes | Basic gains/mix supported; planner/catalog/source-role integration outstanding. KRI-132 adds a montage-family recorded-voiceover lane (`compile_phone_montage_plan`, `VoiceoverRenderAsset`, `AudioMixRecipe.narration_asset_id`) behind `PHONE_NARRATION_RENDERING_ENABLED` (default on since the KRI-132 rollout) — footage/matched-track bed gain mirrors the cloud's `_mix_user_voiceover` exactly; the cloud's `loudnorm` normalization pass has no phone equivalent (a known, accepted difference). Side-chain-ducked original-audio narration beds (the `narrated`/`narrated_*` archetypes) and the guided-story narration lane remain cloud-only. |
...
- 30-fps preview, seek p95 <=250 ms, 60-second export <=120 seconds on iPhone 13
  and a current iPhone, recording memory/thermal state. General rollout requires
  these measurements; the authorized account pilot is experimental. All in-scope
  rows must pass for KRI-29 completion.
