...
| Transitions | `crossfade`, `fade_black`, `fade_white`, `wipe_left`, `wipe_right` (`pipeline/transitions.py`), plus semantic hard/match cuts, speed ramps and curtain-close | Five native render paths; 40 production-FFmpeg grayscale references pass preview/export/backward seeking. Extended capability is not advertised; guided compiler still rejects transitions. Colored moving footage, source audio and phone pilot remain open ([details](clip-transitions.md)) |
