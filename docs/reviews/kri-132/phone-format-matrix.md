...
> **Update — talking-to-camera + narrated PR (stacked on the voiceover PR).** Two more rows move from **Blocked** to **Works**: (1) `subtitled` ("Talking to camera": exactly one portrait clip, own audio → editable captions) compiles through `app/pipeline/phone_subtitled_plan.py::compile_phone_subtitled_plan` via a new `_run_phone_subtitled_job`, behind `phone_subtitled_rendering_enabled` (default on) + `subtitled_archetype_enabled`; (2) `narrated` / `narrated_planned` / `narrated_ready` **with a recorded voiceover** compile through `app/pipeline/phone_narrated_plan.py::compile_phone_narrated_plan` via a new `_run_phone_narrated_job`, behind `phone_narrated_rendering_enabled` (default on) + the SAME `phone_narration_rendering_enabled` + verified `narrationAudio` the montage-family voiceover render already uses. A narrated item with NO voiceover (self-narration) is also let through the dispatch gate for the single-clip shape only (multi-clip self-narration would need `talking_head`, which still has no phone compiler) and is routed to `_run_phone_subtitled_job`, which re-verifies the real `_resolve_archetype` outcome post-ingest and fails closed (`UnsupportedPhonePlan`) if it lands on anything but `subtitled`. `talking_head` itself (any clip count) and `slides` remain **Blocked** — no phone compiler exists for either. The single source of truth for "which formats can render on the phone right now" is `app.services.phone_rollout.phone_render_supported_formats()` — `PHONE_RENDER_SUPPORTED_FORMATS` (`edit_format.py`) is now a broader, settings-free "has a compiler at all" constant that this new helper intersects with the live rollout flags; every runtime consumer (dispatch gate, worker dispatch fork, `/capabilities` picker, and the Main Creator Agent's `phone_format:{format}` manifest capability) now calls the helper instead of reading the constant directly. New typed phone-gate reasons: `narrated_voiceover_unavailable` (narrated + voiceover, flags off) and `subtitled_clip_count_unsupported` (subtitled item with != 1 clip). Known divergences from the cloud path, accepted for v1: no silence-cut/speech-cleanup on either lane (a `required_v1` speech-cleanup contract fails closed instead of silently skipping cleanup — see `_phone_speech_cleanup_contract_guard`), no agentic storyboard re-ranking for narrated clip assignment (script/guide order only), no side-chain ducking/loudnorm/voice fade-out (flat bed gain, same approximation the montage-family voiceover render already ships), no face-tracked crop (subtitled requires a portrait source clip instead), and `talking_head` (2+-clip self-narration) is still entirely unsupported on the phone.

All line numbers were verified against `origin/main` at commit `88eb0e5` (2026-09-21) by grep, not copied from memory. Line numbers in the two Update blocks above may drift from the base doc as this repo continues to change; treat them as approximate pointers, not a live index.
