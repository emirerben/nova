...
Text tab opens without taking keyboard focus; tapping its input starts typing.
Below the input it lists every on-screen text block under "On screen" (KRI-185):
the title, one label per clip and the creator's own text, in time order and
...
when there is more than one block. Its unfinished draft survives tab switches,
...
`NativeEditorInspectorUITests` covers panel switching (including Text), dragging
the visible panel grabber up and back down, compact presets, preview
