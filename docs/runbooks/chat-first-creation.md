...
| --- | --- | --- |
| Montage | `montage` / Classic | video, image, or audio media |
...
Editor tabs after a cut is ready. The picker accepts `video/*,image/*,audio/*`.
...
Deploy the API migration and route first. Then deploy the worker/API code that
can create and reconcile threads, and finally deploy the web client. This order
lets a new web client receive a deliberate backend `404` capability response and
render its legacy rollback experience during a short deploy skew. Network and
5xx errors are user-visible errors; they must not silently fall back.
