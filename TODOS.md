...
**What:** Replace the hardcoded `[0, min(clip_dur, 5/10/15)]` windows in `_fallback_moments()` (`app/tasks/template_orchestrate.py`) with content-aware moments derived from the Whisper transcript — e.g., longest speech bursts, word-density peaks. As of v0.40.0.1 the windows carry a content-derived `description` (`_synthetic_moment_description`: detected_subject/hook_text), but window *timing* is still duration buckets.
