...
- `SILENCE_CUT_ENABLED` — defaults **`false`**. Auto-cuts silences + fillers ("uh", "ııı") from speech render paths ONLY (subtitled + talking_head spine; self-narration inherits; music/beat paths structurally excluded — guard: `tests/tasks/test_silence_cut_isolation.py`). Engine: `app/pipeline/silence_cut.py` (pure CutPlan) inside `reframe_and_export(keep_segments=…)` with alternating punch-in; captions rebuilt from remapped words, fillers stripped. Fail-open: any failure ⇒ uncut render. `RETAKE_CUT_ENABLED` remains an independent default-false kill switch. Launch BOTH together only after TR+EN production-image parity passes; either may roll back independently. Reviewable candidates use ownership/revision-guarded public apply/restore actions and a dedicated full speech rerender; no browser-side cut. Behavior pin: `tests/pipeline/test_silence_cut_golden.py`; plans/010. Apply both: `fly secrets set SILENCE_CUT_ENABLED=true RETAKE_CUT_ENABLED=true --app nova-video` + worker restart.
