...
- `SMART_MUSIC_BED_REQUIRES_REQUEST_ENABLED` — default `true` (KRI-20). Bed resolver only returns a creator-selected treatment (editor picker); never auto-matches. `false` restores pre-fix auto-match. Guards: `test_smart_music_bed_requires_explicit_request`, `test_persisted_music_treatment_survives_rerender`. Apply: `fly secrets set …=false --app nova-video` + worker restart.
