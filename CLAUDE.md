...
- **Layer-2 cache namespace:** `template_cache.TEXT_OVERLAY_VERSION_V2 = "v2-2026-05-19"`. Bumping this string orphans every pre-fix Layer-2 cache entry (force_layer2 builds only — `recipe-only` manual templates use a separate namespace and are untouched); next access reanalyzes with the fixed pipeline. Bump this string whenever the Layer-2 stages change semantics in a way that would taint cached recipes.
