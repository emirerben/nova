...
- `FULLSCREEN_CUTAWAYS_ENABLED` — defaults **`false`**. Gates ONLY the AI fullscreen branch (`build_suggestions` slot `"full"` → `display_mode="fullscreen"` cover-crop takeover); the manual popover toggle is ungated; render rollback = `MEDIA_OVERLAYS_ENABLED`. Flip on Fly ONLY after the plan-009 web surfaces are live on Vercel (preview/bake parity). Rule (g) fails closed on assets without ANALYSIS_VERSION-3 pixel dims — the backfill re-analyzes them on first match. Guards: `tests/test_overlay_fullscreen_rules.py` (kill-switch byte-identical off), dual preset pins in `tests/test_media_overlay_command.py` (fullscreen pass = `preset=fast` + 1200s timeout + one-shot veryfast retry — see plans/009).
