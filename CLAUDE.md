...
- `RECONCILE_STUCK_VARIANTS_ENABLED` — default `true`. Kill switch for the stuck-variant watchdog (`reconcile_stuck_variants`), which WRITES user-visible state. **Invariant:** every reaper jsonpath is param-bound and executed against real Postgres by `tests/tasks/test_reaper_jsonpath.py` — Postgres parses jsonpath at execution time, so a mocked test cannot catch a malformed one (v0.69.1.0: agents/DECISIONS.md). Migration 0099's index predicate must stay byte-identical to `_STUCK_VARIANT_JSONPATH`. Apply: `fly secrets set RECONCILE_STUCK_VARIANTS_ENABLED=false` + worker restart.
- `SILENCE_CUT_ENABLED` / `RETAKE_CUT_ENABLED` — default `false`; speech paths only, fail-open. `SPEECH_CLEANUP_BUDGET_CLAMP_ENABLED` `true` = clamp not `unsafe_plan`. `SPEECH_CLEANUP_MAX_REMOVAL_FRAC_REQUIRED` `1.0` = no fraction cap (`MIN_OUTPUT_S` 3.0 s the only rail; rollback `=0.55`; auto `MAX_REMOVAL_FRAC` 0.4 differs), safe ONLY because rule 0 carves one dominated interior span (no edge trims) + `MIN_KEEP_SPEECH_SEGMENT_S` replaced the safety the cap gave by chance: never re-cap a "cut a word" bug. Both + `DETECTOR_VERSION` (bump on ANY change) are in the policy fingerprint; a flip retires analyses, re-consents, reshuffles cohorts. Pins `test_silence_cut*.py` (`TestRuleZeroCannotCutRealSpeech`); plans/010/019/021.
