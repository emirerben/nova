...
- **Renderer-parity invariant (the #296 class — "looks right locally, clips in prod").** Any overlay field plumbed through the burn entry dict MUST be honored by BOTH renderers, or it silently drops on whichever path it missed. The trap: the admin overlay-preview (`render_overlays_at_time`) and classic templates both render via Pillow, while agentic templates + music jobs render via Skia — so a Pillow-only fix passes every local check (preview, Pillow unit tests) while the burned agentic/music video stays broken. History: #296 carried `text_anchor` through the burn dict and fixed Pillow + preview, but Skia's `_draw_centered_text`/`_draw_karaoke_line` kept centering every line on `position_x_frac`; prod job `ff0d2e1c` still rendered `text_anchor="left"` overlays clipped ("It's not just luck" → "s not just luck"). #297 fixed `_draw_centered_text`/`_draw_pop_in_with_suffix`; the karaoke path was fixed later. **Guard:** `test_both_renderers_honor_text_anchor_left` in `tests/pipeline/test_text_overlay_skia.py` renders the same overlay through both renderers and asserts they place the anchor identically — extend it when adding any new anchor/position field. **Verification rule:** an agentic/music overlay change is NOT verified by the admin preview — verify the actual burned (Skia) video. **Tool:** `make verify-overlays` is that rule as code — see "Pre-PR text-overlay verify" below.
- **Pre-PR text-overlay verify (`make verify-overlays`).** Before opening any PR that touches the Skia renderer (`text_overlay_skia.py`), overlay layout, or anything plumbed through the burn dict, run the gate locally. It renders a recipe's overlays through the REAL Skia path inside the prod Docker image (`docker-compose.local-render.yml` → repo-root `Dockerfile`, so fonts + ffmpeg match prod), then asserts each overlay renders un-clipped and writes a contact-sheet `montage.png` + `report.json` to `.overlay-verify/`. The clipping check (opaque-pixel bbox vs frame edges) is the primary, font-independent signal — it catches the #296 class (left/right anchor clipping, the "s not just luck" bug) and exits non-zero so it gates a PR. Content (does the text read correctly?) is the montage: review it, or have the agent read it (Claude reads PNGs) — that beats tesseract on stylized fonts, and tesseract is not in the prod image. Commands: `make verify-overlays ARGS="--fixtures"` (the committed regression set under `tests/fixtures/overlay_verify/`), `ARGS="--recipe <path>"`, or `ARGS="--template <uuid>"`. Optional automated OCR content match runs on the host after: `cd src/apps/api && python -m app.cli.verify_overlays --stage ocr --out ../../../.overlay-verify`. Library: `app/pipeline/overlay_verify.py`; CLI: `app.cli.verify_overlays` (inside the image because the prod `Dockerfile` copies `app/`, not `scripts/`); guard test: `tests/pipeline/test_overlay_verify.py` (detector unit tests on synthetic clipped frames prove the verifier actually catches bugs, plus a fixture-render regression suite). **Source of truth is the recipe** — this verifies the renderer faithfully burns what the recipe declares; it does NOT check whether the agent extracted the right text (separate problem) or orchestrator timing clamps (curtain-exit/merge/beat-snap) that a full job applies. **Agent pre-PR gate:** run it, read `report.json` + view `montage.png`, fix any FAIL, only then `/ship`.
...
- **Layer-2 text-overlay pipeline** — a multi-stage OCR pipeline replacing the single Gemini call when `settings.text_overlay_v2_enabled=True` (default `False`) or when an admin hits `reanalyze-agentic?use_layer2=true`. Stages: A (frame extraction) → B (OCR) → C (temporal grouping) → D (phrase reconstruction) → E (transcript alignment) → F (classification) → G (output conversion). Entry point: `run_full_pipeline()` in `app/pipeline/text_overlay_v2/pipeline.py`; returns `TemplateTextOutput` (same schema as Layer-1). Routing lives in `TemplateTextAgent.run()` — when the flag is `True` it calls `_run_layer2()` which downloads the template video and invokes `run_full_pipeline()`; when `False` the original single-call path runs unchanged. Eval fixtures for the Layer-2 path are a separate follow-up PR — until then, run evals against Layer-1 (flag off).
- **Transcript wiring (Layer-2 stage E):** `agentic_template_build_task` runs `_fetch_transcript_words_for_layer2` before `extract_template_text_overlays` — invokes `TranscriptAgent` against the template's Gemini file_ref and forwards the per-word list as the `transcript_words` kwarg on `template_text_extraction.extract_template_text_overlays`. Gated on `use_layer2 or settings.text_overlay_v2_enabled` so Layer-1 builds skip the cost. Best-effort: transcribe failures fall back to `[]` and stage E passthrough; programming errors (`TypeError`, `AttributeError`, `ImportError`, `NameError`) re-raise so silent regressions can't hide. Before this wiring, stage E was dead code in prod because the empty-transcript early return always fired.
- **`nova.compose.text_alignment`** (Layer-2 stage E) — rewrites OCR phrase text using the Whisper transcript as ground truth (transcript-authoritative, not "fuzzy correct"). Input: `Phrase[]` from stage D + per-word transcript. Output: same phrases with text replaced verbatim from transcript words; unmatched phrases (hallucinations) dropped. Agent file: `app/agents/text_alignment.py`; schemas: `app/agents/_schemas/text_alignment.py`; prompt: `prompts/align_overlay_to_transcript.txt` (`prompt_version` `2026-05-19`). `_sanitize_aligned_line` strips ASS subtitle tags (`{\an5}`, `{\fs120}` etc. — would otherwise reach the renderer as positioning/style overrides instead of text), Unicode control + format codepoints (RTL overrides, ZWJ, line/paragraph separators), `\n`/`\N` literals, and debug markers (`[overlap_truncated]`). Does NOT dedup adjacent identical tokens — legitimate refrains ("rain rain go away") must survive.
- **Stage D `_finalize` (`text_overlay_v2/phrases.py`) dedups same-text events** within a phrase by casefolded-stripped key, collapsing OCR re-firing of the same word across frames into a single line. Atomized phrases (the prod default) hit the single-event short-circuit and skip the loop.
- **Stage G (`pipeline._classified_phrases_to_output`) normalizes `sample_text`** — strips embedded `\n`, literal `\\n`/`\\N` escape sequences, and `[overlap_truncated]` markers before building the overlay so debug markers can't render on screen.
- **Layer-2 cache namespace:** `template_cache.TEXT_OVERLAY_VERSION_V2` (a dated string, currently `v2-2026-05-23c-...`). Bumping it orphans every pre-fix Layer-2 cache entry (force_layer2 builds only — `recipe-only` manual templates use a separate namespace and are untouched); next access reanalyzes with the fixed pipeline. **Bump it whenever any Layer-2 stage changes semantics in a way that would taint cached recipes — it is the ONLY thing that invalidates Layer-2 cache.** The Stage E/F agent `prompt_version`s (`text_alignment`, `text_classification`) are NOT in the cache key, so editing those prompts without bumping the constant is invisible in prod (every access cache-hits a pre-change recipe — the "looks right locally, stale in prod" trap). A CI guard enforces this: `.github/workflows/layer2-cache-guard.yml` (script `src/apps/api/scripts/check_layer2_cache_bump.sh`) fails any PR touching `text_overlay_v2/`, the Stage E/F agents/schemas, or their prompts without a bump. Escape hatch for genuinely non-semantic edits: `[skip-layer2-cache-bump]` in a commit message.
- **Layer-2 OCR backend divergence (local ≠ prod):** Stage B picks its OCR backend at runtime via `default_backend()` in `app/services/text_overlay_ocr.py` — Cloud Vision when `GOOGLE_SERVICE_ACCOUNT_JSON`/`GOOGLE_APPLICATION_CREDENTIALS` is set (prod), else Apple Vision on macOS (local dev). The two return different bounding boxes, confidence, and line reconstructions, so a Layer-2 result verified locally on Apple Vision will NOT match the prod (Cloud Vision) render. Generate Layer-2 eval fixtures against Cloud Vision, or pin the backend via the `backend=` kwarg on `run_full_pipeline()`, rather than trusting a locally-captured fixture.
