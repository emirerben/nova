...
Localhost sign-in bypasses Google consent via the `dev-login` NextAuth provider, gated behind `ALLOW_DEV_LOGIN=true` + `INTERNAL_API_KEY` in the repo-root `.env` (works in every worktree; `dev-auto.sh` sources it). NEVER set `ALLOW_DEV_LOGIN` in Vercel/Fly. Setup + gotchas: `docs/runbooks/local-dev-login.md`.
...
- `EDITORIAL_SEQUENCE_ENABLED` — default `true`. Editorial variants with coherent original speech render the transcript-synced sequence; otherwise RHYTHM MODE (agent failure ⇒ static cluster, never heuristic). `false` ⇒ legacy static cluster, byte-identical. Guards: `tests/tasks/test_generative_build_sequence.py`. Apply: fly secret + restart. Detail: agents/DECISIONS.md "CLAUDE.md flag detail".
...
- `SMART_MUSIC_BED_REQUIRES_REQUEST_ENABLED` — default `true` (KRI-20). Bed resolver only returns a creator-selected treatment; never auto-matches. `false` restores pre-fix behavior. Apply: `fly secrets set …=false` + restart.
- `SOUND_EFFECTS_ENABLED` / `MEDIA_OVERLAYS_ENABLED` — default **`false`**; gate the SFX/overlay lane routes in `routes/plan_items.py` (404 off). **Dual-flag trap:** `NEXT_PUBLIC_` twins on Vercel; frontend on + backend off ⇒ saves 404. Fly first, then Vercel; restart api + worker. Detail: `docs/pipelines/generative.md`, DECISIONS.md "CLAUDE.md flag detail".
...
- `FULLSCREEN_CUTAWAYS_ENABLED` — default **`false`**; AI fullscreen branch. Dual-flag with `NEXT_PUBLIC_FULLSCREEN_CUTAWAYS_ENABLED`: **Fly first, then Vercel** (new web + OLD api bakes manual fullscreen as pip). Rollback = `MEDIA_OVERLAYS_ENABLED`. Guards: `tests/test_overlay_fullscreen_rules.py`, `tests/test_media_overlay_command.py`; plans/009.
- `SUBTITLED_TEXT_LANE_ENABLED` — default **`false`**; text burns onto the caption-free base first, captions last (`_compose_subtitled_final`); fast-reburns mint a NEW GCS key. Dual-flag `NEXT_PUBLIC_SUBTITLED_TEXT_LANE_ENABLED`; Fly first, then Vercel; restart api + worker. Internals: `docs/pipelines/generative.md`.
...
- `POSTER_ONDEMAND_REPAIR_ENABLED` / `POSTER_REPAIR_QUEUE` — default **`false`** / `celery`; enqueues `tasks.repair_job_poster`. **Set the queue FIRST** — needs `autoplace-jobs` (2GB), never the 1GB `light` machine. Guards: `tests/tasks/test_poster_repair.py`, `tests/routes/test_me_jobs.py`. Apply order: DECISIONS.md "Storage retention incidents".
...
- `SLIDE_POSTS_ENABLED` — defaults `true`. Ordered mixed-media (image+video) posts; see docs/pipelines/slide-post.md. Rollback: `fly secrets set SLIDE_POSTS_ENABLED=false --app nova-video` + restart (api+worker).
