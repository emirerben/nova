...
- **Layer-2 text-overlay pipeline** — a multi-stage OCR pipeline replacing the single Gemini call when `settings.text_overlay_v2_enabled=True` (default `False`). Stages: A (frame extraction) → B (OCR) → C (temporal grouping) → D (phrase reconstruction) → E (transcript alignment) → F (classification) → G (output conversion). Entry point: `run_full_pipeline()` in `app/pipeline/text_overlay_v2/pipeline.py`; returns `TemplateTextOutput` (same schema as Layer-1). Routing lives in `TemplateTextAgent.run()` — when the flag is `True` it calls `_run_layer2()` which downloads the template video and invokes `run_full_pipeline()`; when `False` the original single-call path runs unchanged. `TemplateTextInput` carries an optional `transcript_words` field forwarded to stage E; existing callers that omit it get an empty list and stage E passes phrases through unchanged. Eval fixtures for the Layer-2 path are a separate follow-up PR — until then, run evals against Layer-1 (flag off).
- **`nova.compose.text_alignment`** (Layer-2 stage E) — corrects OCR phrase text using the Whisper transcript as ground truth. Input: `Phrase[]` from stage D + per-word transcript. Output: same phrases with OCR errors fixed; unmatched phrases (hallucinations) dropped. Agent file: `app/agents/text_alignment.py`; schemas: `app/agents/_schemas/text_alignment.py`; prompt: `prompts/align_overlay_to_transcript.txt`.
