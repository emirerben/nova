...
final class DissolvePainterTests: XCTestCase {
    #if canImport(AVFoundation)
    @MainActor func testDissolveUsesCompositionTimeInPreviewAndExport() async throws {
        ...
        let exportCases = try rows.prefix(1).map { row in
            ...
            layer["effect"] = "dissolve-out"; layer["staggered"] = NSNull(); layer["dissolve_seed"] = 138
        ...
        for (index, test) in exportCases.enumerated() {
            ...
            for generator in [reference, exported] {
                ...
                XCTAssertGreaterThan(counts[1], 0); XCTAssertLessThan(counts[2], counts[1])
            }
        }
    }
