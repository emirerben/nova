...
final class StaggeredPainterTests: XCTestCase {
    private struct Case: Decodable { let layer: PortableTextLayer; let samples: [Sample] }
    private struct Sample: Decodable { let time: Double; let settled: Bool; let calls: [Run] }
    private struct Run: Decodable { let text: String; let x: Double; let baselineY: Double; let alpha: Int; let rotation: Double; let translations: [[Double]] }
    private func cases() throws -> [Case] {
        let fixture = URL(fileURLWithPath: #filePath).deletingLastPathComponent()
            .appendingPathComponent("../../../../../api/tests/fixtures/phone_staggered_layout.json").standardizedFileURL
        return try RecipeJSON.decoder().decode([Case].self, from: Data(contentsOf: fixture))
    }
    func testGlyphPositionsAndTransformsMatchActualCloudDrawing() throws {
        let cases = try cases()
        XCTAssertEqual(cases.count, 6)
        for test in cases {
            let layer = test.layer
            let content = try XCTUnwrap(layer.staggered)
            try content.validate()
            for sample in test.samples {
                let state = try StaggeredSliceTiming.sample(text: content.text, localTime: sample.time, duration: layer.end - layer.start, motion: layer.motion)
                XCTAssertEqual(state.settled, sample.settled)
                if state.settled { continue }
                let visible = content.glyphs.filter { state.lines[$0.logicalLine].glyphs[$0.glyphIndex].opacity > 0.001 }
                XCTAssertEqual(visible.count, sample.calls.count)
                for (entry, expected) in zip(visible, sample.calls) {
                    let glyph = state.lines[entry.logicalLine].glyphs[entry.glyphIndex]
                    XCTAssertEqual(entry.run.text, expected.text)
                    XCTAssertEqual(entry.run.x, expected.x, accuracy: 1e-5)
                    XCTAssertEqual(entry.run.baselineY, expected.baselineY, accuracy: 1e-5)
                    XCTAssertEqual(Int(255 * glyph.opacity), expected.alpha)
                    XCTAssertEqual(glyph.rotateDeg, expected.rotation, accuracy: 1e-8)
                    XCTAssertEqual(entry.pivotX, expected.translations[0][0], accuracy: 1e-5)
                    XCTAssertEqual(entry.pivotY + entry.run.fontSize * glyph.translateYEm, expected.translations[0][1], accuracy: 1e-5)
                    XCTAssertEqual(-entry.pivotX, expected.translations[1][0], accuracy: 1e-5)
                    XCTAssertEqual(-entry.pivotY, expected.translations[1][1], accuracy: 1e-5)
                }
            }
        }
    }
    ...
    @MainActor func testRevealUsesCompositionTimeInPreviewAndExport() async throws {
        ...
        let exportCases = try cases()
        XCTAssertEqual(exportCases.count, 6)
    ...
    func testPartialRotatedTextPaintsAndSettlesWithoutMovingItsBitmap() throws {
        ...
        for test in try cases() {
            ...
            let painter = try XCTUnwrap(painted.staggered)
