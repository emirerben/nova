...
enum NativeSkiaShadowPainter {
    ...
    /// Non-private so a caller that cached a `tile(...)` result (see below)
    /// can re-composite it into a fresh frame without recomputing it.
    static func composite(_ image: CGImage, in rect: CGRect, onto context: CGContext, bounds: CGRect) throws {
        try RenderProfiler.measure("shadow.composite", units: Int(rect.width * rect.height)) {
    ...
    /// Builds the blurred, tinted shadow tile for one stroke/glyph and one
    /// blur layer, without compositing it anywhere. `draw` below is a thin
    /// wrapper around this plus `composite`/`context.draw` — identical
    /// behavior, split out so a stateful caller (`NativeGiantHandwritingPainter`)
    /// can cache the returned tile across frames and skip straight to
    /// `composite` on a cache hit, instead of rebuilding and re-blurring an
    /// identical mask every frame.
    static func tile(pathBounds: CGRect, blur: TextBlurLayer, bounds: CGRect, rotation: CGAffineTransform,
        transform: CGAffineTransform, maxBitmapBytes: Int, opacity: Double, imageContext: CIContext,
        drawMask: (CGContext) -> Void) throws -> (image: CGImage, crop: CGRect)? {
        try RenderProfiler.measure("shadow.tile") { () throws -> (image: CGImage, crop: CGRect)? in
            ...
            guard maskBounds.width * maskBounds.height * 4 <= Double(maxBitmapBytes),
                  ...
                  }) else { throw NativePreviewFeatureError("NativeSkiaShadowPainter-74") }
            mask.translateBy(x: -maskBounds.minX, y: -maskBounds.minY)
            mask.concatenate(placement)
            guard let image = RenderProfiler.measure("shadow.mask.stroke", { () -> CGImage? in
                drawMask(mask)
                return mask.makeImage()
            }) else { throw MediaEngineError.exportFailed }
            var shadow = try RenderProfiler.measure("shadow.ci.graph") { () throws -> CIImage in
            ...
            if crop.isNull || crop.isEmpty { return nil }
            guard let output = RenderProfiler.measure("shadow.gpu.sync", units: Int(crop.width * crop.height), { () -> CGImage? in
                imageContext.createCGImage(shadow, from: crop, format: .RGBA8, colorSpace: CGColorSpace(name: CGColorSpace.sRGB)!)
            }) else { throw MediaEngineError.exportFailed }
            return (output, crop)
        }
    }
    ...
    static func draw(pathBounds: CGRect, blur: TextBlurLayer, context: CGContext,
        ...
        guard let (output, crop) = try tile(pathBounds: pathBounds, blur: blur, bounds: bounds, rotation: rotation,
            transform: transform, maxBitmapBytes: maxBitmapBytes, opacity: opacity, imageContext: imageContext,
            drawMask: drawMask) else { return }
