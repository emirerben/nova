...
final class NativeDissolveRenderer: @unchecked Sendable {
    ...
    private let preset: DissolveTiming.Preset
    ...
    init(width: Int, height: Int, seed: UInt32, maxBitmapBytes: Int, preset: DissolveTiming.Preset = .text) throws {
        ...
        guard (1...4096).contains(width), (1...4096).contains(height),
              width * height * 8 <= maxBitmapBytes else { throw NativePreviewFeatureError("NativeDissolveRenderer-16") }
        warp = try NativeDissolveWarp(width: width, height: height, seed: seed, maxBitmapBytes: maxBitmapBytes / 2, preset: preset)
        bitmapBytes = width * height * 8
        var noise = [Float](repeating: 0, count: width * height)
        for y in 0..<height {
            for x in 0..<width {
                noise[y * width + x] = DissolveTiming.particleNoise(cellX: UInt32(x / (preset == .media ? 1 : 3)), cellY: UInt32(y / (preset == .media ? 1 : 3)), seed: seed)
            }
        }
        ...
        """)
        guard let finish = kernels.first else { throw NativePreviewFeatureError("NativeDissolveRenderer-44") }
        self.finish = finish
    }

    func image(source: CIImage, localTime: Double, duration: Double) throws -> CIImage {
        let state = try DissolveTiming.sample(localTime: localTime, duration: duration, preset: preset)
        ...
        let start = preset == .media ? 0.42 : 0.18
        let breakup = state.progress <= start ? -1 : min(1, (state.progress - start) / 0.72)
        ...
        guard let image = finish.apply(extent: warp.extent, roiCallback: { _, rect in rect },
            ...
            throw NativePreviewFeatureError("NativeDissolveRenderer-60")
        }
        return image
    }
}
