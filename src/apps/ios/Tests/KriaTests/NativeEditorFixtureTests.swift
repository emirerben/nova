...
final class NativeEditorFixtureTests: XCTestCase {
    ...
    func testAllAccountFreeFixtureShapesAreDeterministic() {
        ...
        XCTAssertEqual(shapes.map(\.rawValue), ["legacy-visuals", "caption-visuals", "projected-captions", "source-text", "two-text", "boundary", "all-lanes", "stress-71", "unknown-sections", "autoscroll-extend", "guided-text"])
        for shape in shapes {
