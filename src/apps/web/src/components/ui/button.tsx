  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        // Alias: "ink" is the pre-migration primary-CTA name. Same look as
        // "default" (the `<InkButton>` wrapper passes `variant="ink"`).
        ink: "bg-primary text-primary-foreground hover:bg-primary/90",
        // Alias: "lime" was the soft-pill secondary action. Same look as
        // "secondary" now that lime is no longer the component-chrome accent.
        lime: "bg-secondary text-secondary-foreground hover:bg-secondary/80",
      },
      size: {
        ...
        icon: "h-9 w-9",
        // Alias: "icon-sm" (dense inspector rows).
        "icon-sm": "h-8 w-8",
        // Alias: "pill" — same footprint as "sm" now that buttons aren't pills.
