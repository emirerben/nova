...
class LibraryRetentionWarning(BaseModel):
    job_id: str
    delete_at: datetime
    source_count: int
...
class LibraryRetentionSummary(BaseModel):
...
class LibraryResponse(BaseModel):
    ...
    retention_warnings: list[LibraryRetentionWarning] = Field(default_factory=list)
...
async def list_my_jobs(
    ...
    # Warning candidates are generated by the report-only mark phase at day
    # 83. The feature flag keeps the pre-rollout endpoint query shape exactly
    # unchanged, while enabled environments surface a durable notice before
    # any approved day-90 deletion can run.
    if settings.storage_retention_enabled:
        warning_predicates = (
            StorageRetentionEntry.creator_id == user.id,
            StorageRetentionEntry.action == "warn",
            StorageRetentionEntry.status == "pending",
            StorageRetentionEntry.job_id.is_not(None),
        )
        summary_row = (
            await db.execute(
                select(
                    func.count(func.distinct(StorageRetentionEntry.job_id)).label(
                        "affected_video_count"
                    ),
                    func.count(StorageRetentionEntry.id).label("source_count"),
                    func.min(StorageRetentionEntry.eligible_at).label("earliest_delete_at"),
                )
                .join(Job, Job.id == StorageRetentionEntry.job_id)
                .where(*warning_predicates, Job.user_id == user.id)
            )
        ).one()
        if summary_row.earliest_delete_at is not None:
            retention_summary = LibraryRetentionSummary(
                affected_video_count=int(summary_row.affected_video_count),
                source_count=int(summary_row.source_count),
                earliest_delete_at=summary_row.earliest_delete_at,
                final_retention_days=settings.storage_retention_final_days,
            )
        warning_rows = (
            await db.execute(
                select(
                    StorageRetentionEntry.job_id,
                    func.min(StorageRetentionEntry.eligible_at).label("delete_at"),
                    func.count(StorageRetentionEntry.id).label("source_count"),
                )
                .join(Job, Job.id == StorageRetentionEntry.job_id)
                .where(
                    *warning_predicates,
                    Job.user_id == user.id,
                    StorageRetentionEntry.job_id.in_([job.id for job in rows]),
                )
                .group_by(StorageRetentionEntry.job_id)
                .order_by(func.min(StorageRetentionEntry.eligible_at).asc())
            )
        ).all()
        retention_warnings = [
            LibraryRetentionWarning(
