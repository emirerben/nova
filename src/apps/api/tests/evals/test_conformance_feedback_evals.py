"""Per-fixture eval gate for nova.plan.conformance_feedback (Creator Agent M4).

Structural-only in CI (skia-free, no LLM). Live mode requires ANTHROPIC_API_KEY
and NOVA_EVAL_MODE=live. Capture a golden fixture with --eval-mode=live before
merging any conformance_feedback prompt change.

Run modes (see tests/evals/README.md):
  pytest tests/evals/test_conformance_feedback_evals.py -v
  pytest tests/evals/test_conformance_feedback_evals.py -v --with-judge
  NOVA_EVAL_MODE=live pytest tests/evals/test_conformance_feedback_evals.py -v \\
...
AGENT_DIR = "conformance_feedback"
AGENT_NAME = "nova.plan.conformance_feedback"
...
def test_conformance_feedback_eval(
    fixture_path: Path,
    eval_mode: str,
    with_judge: bool,
    judge_for,
