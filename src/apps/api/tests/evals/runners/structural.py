...
def check_overlay_format_matcher(output: OverlayFormatMatcherOutput) -> list[str]:
    ...
    if not isinstance(output.behind_subject, bool):
        failures.append(f"behind_subject={output.behind_subject!r} is not a bool")
...
def check_edit_copilot(output: Any) -> list[str]:
    ...
    valid_ops = {
        ...
        # KRI-191: server-only, fact-grounded per-clip labels.
        "label_each_clip",
        "set_text_timing",
        "add_text",
        "remove_text",
