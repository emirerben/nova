"""Per-fixture eval gate for nova.plan.style_intent (Creator Agent M2).

Structural-only in CI (no Skia, no LLM, no network). Live mode requires
ANTHROPIC_API_KEY and NOVA_EVAL_MODE=live. Capture a golden fixture with
--eval-mode=live before merging any prompt change.

Run modes (see tests/evals/README.md):
  pytest tests/evals/test_style_intent_evals.py -v
  pytest tests/evals/test_style_intent_evals.py -v --with-judge
  NOVA_EVAL_MODE=live pytest tests/evals/test_style_intent_evals.py -v \\
    --eval-mode=live --with-judge

EVAL SKIA RULE: this file must NOT import skia or overlay_sizing at the top
level — eval CI lacks libEGL. All style-related imports stay inside test bodies
if needed.
"""
...
AGENT_DIR = "style_intent"
AGENT_NAME = "nova.plan.style_intent"
