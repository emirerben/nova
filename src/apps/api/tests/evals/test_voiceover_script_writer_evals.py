"""Per-fixture eval gate for nova.voiceover.script_writer ("Get a transcript" helper).
...
(check_voiceover_script_writer — spoken-length band + no URLs/handles + non-empty
lines). Live mode requires GEMINI_API_KEY; the judge additionally ANTHROPIC_API_KEY.

Run modes (see tests/evals/README.md):
  pytest tests/evals/test_voiceover_script_writer_evals.py -v
  NOVA_EVAL_MODE=live pytest tests/evals/test_voiceover_script_writer_evals.py -v --eval-mode=live
"""
...
AGENT_DIR = "voiceover_script_writer"
AGENT_NAME = "nova.voiceover.script_writer"
...
def test_voiceover_script_writer_eval(
    fixture_path: Path,
    eval_mode: str,
    with_judge: bool,
    judge_for,
