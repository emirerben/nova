"""Per-fixture eval gate for nova.audio.music_matcher (music_matcher).

Run modes (see tests/evals/README.md for full guide):
  pytest tests/evals/test_music_matcher_evals.py -v
  pytest tests/evals/test_music_matcher_evals.py -v --with-judge
  pytest tests/evals/test_music_matcher_evals.py -v --eval-mode=live --with-judge
"""
...
AGENT_DIR = "music_matcher"
AGENT_NAME = "nova.audio.music_matcher"
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "add hand-authored golden fixtures or run `python scripts/export_eval_fixtures.py` "
        "once a prod run has been recorded"
    ),
)
@pytest.mark.parametrize("fixture_path", FIXTURE_PATHS, ids=lambda p: f"{p.parent.name}/{p.stem}")
def test_music_matcher_eval(
    fixture_path: Path,
    eval_mode: str,
    with_judge: bool,
    judge_for,
