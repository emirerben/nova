...
class TestClipMetadataParse:
    def test_parse_happy_path(self):
        ...
        assert out.brands == ["Coca-Cola"]

    def test_parse_unwraps_single_object_array(self):
        raw = json.dumps(
            [
                {
                    "transcript": "hello",
        ...
        assert out.hook_score == pytest.approx(7.0)

    @pytest.mark.parametrize("raw", ["[]", "[{}, {}]", '["nope"]'])
    def test_parse_rejects_other_top_level_arrays(self, raw):
        with pytest.raises(SchemaError, match="response is not a JSON object"):
