...
The model emitted "~7 OF ~6" labels and padded the interview past its own
estimate — arithmetic promises can't be left to the LLM. parse() now derives
the label from the route's turn counter and force-closes at question 7.
"""
...
def test_label_never_advertises_total_below_current_turn():
    # The prod screenshot: question 7 labelled "~7 OF ~6".
    out = _parse(7, label="~7 OF ~6")
    assert out.turn_label == "~7 OF ~7"


def test_question_seven_is_always_final():
    out = _parse(_FORCE_FINAL_AT, label="~7 OF ~8", is_final=False)
...
def test_model_final_label_collapses_to_n():
    out = _parse(5, label="~5 OF ~7", is_final=True)
    assert out.is_final is True
...
def test_unparseable_label_falls_back_to_default_estimate():
    ...
    assert out.turn_label == "~2 OF ~6"


def test_honest_model_label_passes_through():
    out = _parse(2, label="~2 OF ~6")
    assert out.turn_label == "~2 OF ~6"
    assert out.is_final is False
