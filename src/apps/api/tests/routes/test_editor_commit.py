...
def test_background_music_commit_persists_separate_bed_for_no_song_variant(monkeypatch):
    ...
    assert v["background_music"]["track_id"] == "t2"
    assert v["background_music"]["track_title"] == "Bed Track"
    assert v["background_music_treatment"]["src_gcs_path"] == "music/t2.mp3"
    assert prep["sections"]["background_music"] is True
    assert prep["sections"]["music"] is False
    assert prep["background_music_changed"] is True
