...
ffmpeg -hide_banner -loglevel error -f lavfi -i testsrc2=size=1080x1920:rate=30:duration=2 -c:v libx264 -preset fast -crf 18 -color_primaries bt709 -color_trc bt709 -colorspace bt709 /tmp/look-parity/source-full.mp4 -y
ffmpeg -hide_banner -loglevel error -i /tmp/look-parity/source-full.mp4 -vf "$LOOK" -c:v libx264 -preset fast -crf 18 -color_primaries bt709 -color_trc bt709 -colorspace bt709 /tmp/look-parity/golden-full.mp4 -y
